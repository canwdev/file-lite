...
function write_image_file(canvas, mime_type, blob_callback) {
	...
	if (bmp_match) {
		...
		show_error_message("TIFF export is not included in this build.");
	} else {
		canvas.toBlob(blob => {
...
function read_image_file(blob, callback) {
	...
	blob.arrayBuffer().then((arrayBuffer) => {
		...
		if (detected_type_id === "bmp") {
			...
			callback(new Error("TIFF import is not included in this build."));
		} else if (detected_type_id === "pdf") {
			callback(new Error("PDF import is not included in this build."));
		} else {
			monochrome = false;
			file_format = {
				// bmp: "image/bmp",
