...
- **功能**
  ...
  - 网络驱动器：SMB / UNC 共享与 WSL 发行版是一等公民 —— 映射盘符、在「此电脑」里添加的网络位置、已安装的 WSL 发行版都会出现在侧边栏，其余路径（`\\server\share`、`\\wsl.localhost\Debian`、本机任意目录）直接在地址栏输入即可。网络位置按更低的并发读取，不可达时报「可重试」而不是假装目录不存在
