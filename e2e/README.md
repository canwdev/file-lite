...
| `12-mounted-folder.spec.ts` | 浏览器挂载的本地文件夹：挂载 / 浏览 / 取消挂载；在卷内新建文件与文件夹、重命名、删除都真的落到目录里；服务端 ⇄ 挂载卷双向复制与移动（源被真的删掉）；只读卷在行上标注并挡住写操作、给出原因；**真 MP3（带内嵌封面）**能解出时长、显示封面并播放；**嵌套子目录**的文件夹预览与面包屑下拉不把挂载路径发给服务端；**接口审计**：不发挂载路径给服务端、无 4xx/5xx、盘列表只拉一次。目录选择框用 OPFS 打桩，断言一路验到真实文件 | `12-mounted-folder` |

合计 67 个用例（另有 1 个跳过，见下），单次运行约 135 秒。
