...
| `12-mounted-folder.spec.ts` | 浏览器挂载的本地文件夹：挂载 / 浏览 / 取消挂载；在卷内新建文件与文件夹、重命名、删除都真的落到目录里；服务端 ⇄ 挂载卷双向复制与移动（源被真的删掉）；只读卷在行上标注并挡住写操作、给出原因；**真 MP3** 能解出时长并播放；**接口审计**：不发挂载路径给服务端、无 4xx/5xx、盘列表只拉一次。目录选择框用 OPFS 打桩，断言一路验到真实文件 | `12-mounted-folder` |

合计 66 个用例（另有 1 个跳过，见下），单次运行约 125 秒。
...
**音乐播放器用例需要真音频**
`tone.mp3` 内联在 `12-mounted-folder.spec.ts` 里（ffmpeg 生成的 1 秒 440Hz，约 4.5KB
base64）。用几个字节的假音频测不出解码问题：浏览器直接报错，用例就变成在测「假文件
当然放不了」。
