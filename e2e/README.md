...
| `08-tabs.spec.ts` | 内置标签页的新增 / 切换 / 保活 / 关闭 / 持久化；最后一个标签不能关；拖拽排序并持久化；右键 Close to the right / Close others；拖文件悬停标签 1s 自动切换且标签不接受落点；`Alt+T` / `Alt+数字` / `Alt+W` | — |
| `09-file-selector.spec.ts` | 选择器模式（打开服务器视频）：单面板、没有标签栏，选中后能返回 | — |

合计 35 个用例，单次运行约 80 秒。
