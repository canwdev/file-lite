...
| `07-drag-drop.spec.ts` | 拖到文件夹行 / 面包屑 / 收藏夹 / 磁盘根 = 移动或复制（同卷移动、跨卷复制、Ctrl 复制，Ctrl 拖回原目录即 duplicate）；文件夹不能拖进自己；拖动收藏项调整顺序并持久化；系统拖入文件上传到这些目录 | `07-drag-drop` |
| `08-tabs.spec.ts` | 内置标签页的新增 / 切换 / 保活 / 关闭 / 持久化；最后一个标签不能关；拖拽排序并持久化；拖文件悬停标签 1s 自动切换且标签不接受落点；`Alt+T` / `Alt+数字` / `Alt+W` | — |

合计 32 个用例，单次运行约 60 秒。
