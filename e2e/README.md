...
| `08-tabs.spec.ts` | 内置标签页的新增（追加在最后）/ 切换 / 保活 / 关闭 / 持久化；最后一个标签不能关；顶栏与工具栏等高；拖拽排序并持久化；右键 Close to the left / right / others；拖到另一个标签的内容区；拖文件悬停标签 1s 自动切换且标签不接受落点；`Alt+T` / `Alt+数字` / `Alt+W` | — |
| `09-file-selector.spec.ts` | 选择器模式（打开服务器视频）：单面板、没有标签栏，选中后能返回 | — |

合计 38 个用例，单次运行约 85 秒。
