...
func (l *ipLimiter) recordFailure(ip string) {
    c := l.failures[ip] + 1
    l.failures[ip] = c
    if c > l.maxAttempts { l.ban(ip) }
}

func (l *ipLimiter) recordSuccess(ip string) { delete(l.failures, ip) }

func (l *ipLimiter) ban(ip string) { l.banned[ip] = time.Now().Add(l.banDuration); delete(l.failures, ip) }

var authLimiter = newIPLimiter()

func AuthMiddleware(next echo.HandlerFunc) echo.HandlerFunc {
    return func(c echo.Context) error {
        if config.Config().NoAuth { return next(c) }
        ip := c.RealIP()
        ...
        if token == "" { token = c.QueryParam("auth") }
