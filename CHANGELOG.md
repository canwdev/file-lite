...
- `safeBaseDir` is back, now opt-in: setting it to a folder restricts the file manager to that folder — anything outside it is refused, and the sidebar only lists locations inside it — while leaving it empty (the default) keeps the whole file system reachable; the folder is checked at startup, so a path that does not exist stops the server instead of silently refusing every request (backend). **If an old config file still has a `safeBaseDir` value it now takes effect again**: previously it was ignored.
- The default stays what it was: with `safeBaseDir` empty the file manager reaches every path the server process can, so a folder anywhere on the machine — including a network share such as `\\server\share` or a WSL distribution at `\\wsl.localhost\Debian` — can be opened by typing its path in the address bar (backend).
