...
> 测试子项目：[`e2e/`](../e2e/README.md)（Playwright，28 个用例，约 50 秒）
...
复制 6000 个小文件时的进度条：`880 / 6000 · 3.45MB / 23.44MB · 15%`
（条目比例与字节比例一致——曾经字节被重复累加，进度会在一半时就冲到 100%）。
...
收藏项本身也能上下拖动来调整顺序（插入线指示落点），顺序写进收藏设置、刷新后保持。
