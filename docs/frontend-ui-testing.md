...
**3. 任务结束后窗口会自动收起，所以断言看磁盘而不是看已经隐藏的行。**
成功的任务跑完，进度窗口就像资源管理器一样收起（行仍留在 store 里，可从状态栏重新打开）。
失败/取消时用失败清单弹窗呈现。因此 `expect.poll(() => fs.readFileSync(...))` 是主力断言，
`toBeHidden()` 用来验证「窗口确实自动收起了」。
...
| 任务窗口居中悬浮且结束后不关闭 | 一个浮动窗口**一直盖住文件列表**，拦截点击 | 全部结束且无失败时自动收起；状态栏加「Tasks」入口可重开 |
...
弹窗期间可以 Cancel：任务继续停在等待状态，从任务行能重新打开，不会丢失决策机会。

![冲突弹窗](../e2e/screenshots/02-conflict-dialog.png)

Cancel 之后从任务行重新打开（`03-conflict-reopen.png`）：

![重新打开冲突弹窗](../e2e/screenshots/03-conflict-reopen.png)

### 任务进度与取消

复制 6000 个小文件时的进度条：`1156 / 6000 · 9.04MB / 23.44MB · 39%`。
取消按钮就在行内；取消后窗口自动收起，测试随即校验目标目录里没有半个文件。
...
| **浏览器 UI** | **`e2e/`（本文）** | **上面全部行为的用户可见路径**：弹窗、进度条、取消、失败清单、跨窗口可见、原地粘贴、下载文件名 |
