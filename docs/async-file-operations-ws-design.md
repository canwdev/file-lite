...
- **前端**：任务 store（快照 / patch / done / 冲突队列、重连对账）、`api/tasks-ws.ts`、element 冲突弹窗（Replace / Skip / Keep both + 逐项或「应用于全部」+ 前后信息对照）、复制 / 移动 / 删除 / 复制副本全部切到任务模型、`fs changed` 驱动目录刷新（取代 `moveRefresh`）、TransferQueue 内的服务端任务进度条与取消 / 冲突入口。
...
- **进度窗口生命周期**：所有任务到终态后自动收起（资源管理器行为），状态栏保留一个「Tasks」入口可重新打开，并显示运行中数量；失败时另有自动弹出的失败清单。
- **测试**：
  ...
  - 前端：独立 Playwright 子项目 `e2e/`，13 个用例约 35 秒，覆盖冲突弹窗、上传冲突、进度条、运行中取消、失败清单与重试、跨窗口可见性；截图与测试方法见 `docs/frontend-ui-testing.md`。
...
3. **任务窗口一直盖住文件列表**：浮动窗口结束后不关闭，拦截列表点击。修复：全部结束且无失败时自动收起 + 状态栏重开入口。
...
- **资源管理器式独立进度窗口 / `More details`**：改为复用现有 TransferQueue 窗口 + 每任务进度条（任务结束后自动收起）；失败清单（`N items failed` + Try Again）已按资源管理器形态实现。
...
### 5.2 进度窗口：`Copying N items…`

对齐资源管理器的进度对话框，复用现有 `ViewPortWindow`（可拖动/缩放）：
...
│                                                          │
│  Name: big.iso                                           │
│  45.2 MB of 200 MB        ·  12.3 MB/s  ·  12s left     │
│                                                          │
│  [▸ More details]                    [ Pause ] [ Cancel ] │
└──────────────────────────────────────────────────────────┘
展开后：
│  ┌────────────────────────────────────────────────────┐  │
│  │ ✅ a.txt          2.4 MB   Done            0.3s    │  │
│  │ ⏳ big.iso      200.0 MB   45%   12.3 MB/s         │  │
│  │ ⏸ b.txt          18 KB    Waiting                  │  │
│  └────────────────────────────────────────────────────┘  │
```

- 标题按 kind：`Copying N items…` / `Moving N items…` / `Deleting N items…` / `Preparing to copy…`（对应 `scanning` 状态）。
- 顶部合并进度条按**字节**计算（无字节信息时退化为条目数）。
- `More details` 展开逐项列表（复用现有 `useVirtualList`）。
- 按钮：`Pause`（→ `paused`，文案变 `Resume`）、`Cancel`。暂停在各文件边界生效，UI 显示「正在暂停…」直到当前文件收尾。
- 速度/剩余时间**在前端按进度增量计算**，不占用服务端字段。
- **全部成功时窗口自动关闭**（对齐 Explorer）；有失败则转为错误对话框。
- 多任务并存时：一个 `TaskCenter` 窗口内分区/分页展示，不每个任务开一个窗口（避免窗口风暴）。
...
| 现有 | 改造 |
| --- | --- |
| `TransferQueue.vue`（`ViewPortWindow` + 虚拟列表 + 进度） | 演进为 `TaskCenter.vue`：单任务时呈现资源管理器式进度对话框；多任务时列表化 |
