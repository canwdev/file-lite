...
| # | 决策 | 结论 |
| --- | --- | --- |
| 1 | `IsPathSafe` 与 `safeBaseDir` | **本次整体删除**（函数 + 配置字段 + 全局变量 + 前后端文档/夹具引用）。**后来按需求部分恢复**：以 `safeBaseDirs`（一组基目录，默认空 = 不限制）回到配置里，判定改在 `fileops.Resolve` 这一个收口处，见 §8.1 |
...
删除 `safeBaseDir` 后默认即全盘可读写。启动输出（`main.go` 的 `applySafeBaseDir`）会打印
`file access scope: the whole file system (no safeBaseDir)`，并在 `docs/config.md` 写清。
这是告知，不是兜底。

**后来 `safeBaseDirs` 按需求回来了**，默认仍为空（不限制），所以上面那句告知依然成立，
只是多了 `file access scope: <路径列表> (safeBaseDirs)` 这一种形态。恢复时的关键取舍：
...
- **复用已有的段边界判定**（`IsWithinRoot` + 兜底的大小写处理），不自己写前缀比较。
  裸 `strings.HasPrefix` 会让 `/srv/files2` 落进基目录 `/srv/files`。
- **形态错误优先于范围错误**：先判路径是否合法（400），再判是否在范围内（403）。
  反过来用户会拿到「超出范围」，而真正的问题是他少写了一个斜杠。
- **多条基目录是并集**，嵌套的折叠成外层那一条（内层不增加可达路径，留着只会让侧边栏重复）。
- **挂载表与盘列表一起收窄**：列表就是枚举面，不收窄等于继续告诉他「这台机器上有什么」。
  多条基目录时没有「唯一根」可推导，所以**把每条基目录本身列成一个位置**。
