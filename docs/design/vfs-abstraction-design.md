...
| # | 决策 | 结论 |
| --- | --- | --- |
| 1 | `IsPathSafe` 与 `safeBaseDir` | **本次整体删除**（函数 + 配置字段 + 全局变量 + 前后端文档/夹具引用）。**后来按需求部分恢复**：以 `allowedRoots`（一组允许根，默认空 = 不限制）回到配置里，判定改在 `fileops.Resolve` 这一个收口处，见 §8.1 |
...
| 浏览器挂载本地文件夹（File System Access API） | 曾实现后在 tag `archive/local-folders` 存档并删除：它把前端变成第二个存储后端——路径命名空间、权限状态机、客户端任务执行器都要另起一套，而浏览器侧的 `move()` 只在 OPFS 上可靠、跨刷新恢复又无法在 CI 里验证，维护面与收益不成比例 |

## 8. 已知风险与开放问题

### 8.1 全盘可访问（语义变更，必须显式告知）

删除 `safeBaseDir`（当时那个必填的单值限制根）后默认即全盘可读写。启动输出
（`main.go` 的 `applyAllowedRoots`）会打印
`file access scope: the whole file system (no allowedRoots)`，并在 `docs/config.md`
写清。这是告知，不是兜底。

**后来访问范围按需求回来了**，作为 `allowedRoots`（默认为空 = 不限制，与上面的默认
完全一致），所以那句告知依然成立，只是多了
`file access scope: <路径列表> (allowedRoots)` 这一种形态。恢复时的关键取舍：

- **判定放在 `fileops.Resolve` 一个地方**，而不是像旧实现那样散在各操作里。旧 `IsPathSafe`
  有三个问题：用 `filepath.Abs`（Windows 上 UNC 会被误判）、用 `filepath.Rel`（跨盘报错即拒，
  合法路径被误伤）、而且**不是一个收口**（读取链路并不都经过它）。这次的实现是加法：
  解析层本来就已经是唯一入口。
- **复用已有的段边界判定**（`IsWithinRoot` + 兜底的大小写处理），不自己写前缀比较。
  裸 `strings.HasPrefix` 会让 `/srv/files2` 落进允许根 `/srv/files`。
- **形态错误优先于范围错误**：先判路径是否合法（400），再判是否在范围内（403）。
  反过来用户会拿到「超出范围」，而真正的问题是他少写了一个斜杠。
- **多条是并集**，嵌套的折叠成外层那一条（内层不增加可达路径，留着只会让侧边栏重复）。
- **挂载表与盘列表一起收窄**：列表就是枚举面，不收窄等于继续告诉他「这台机器上有什么」。
  多条时没有「唯一根」可推导，所以**把每一条本身列成一个位置**。
  关键点：判「这个位置是不是已经在枚举结果里」必须是**相等**而不是前缀包含，
  而且**不能去问挂载表**——挂载表正是由这个函数的结果建立的，问它就会让第二次调用
  答「已经有了」，于是原样返回全盘列表（实测踩过，且单看一次调用发现不了）。
- **源可以在范围外，目标必须在范围内**。否则没法把别处的文件拷进受控目录，而那正是
  这个特性的主要用途。
- **明确不做的**：范围内的软链指向范围外不拦（canonical 不解析软链，决策 10），
  Zip Slip 仍归将来的 ZIP 后端（§8.3），进程权限决定上限（这不是沙箱）。
