...
| # | 决策 | 结论 |
| --- | --- | --- |
| 1 | `IsPathSafe` 与 `safeBaseDir` | **本次整体删除**（函数 + 配置字段 + 全局变量 + 前后端文档/夹具引用）。**后来按需求部分恢复**：`safeBaseDir` 以「默认空 = 不限制」的形式回到配置里，判定改在 `fileops.Resolve` 这一个收口处，见 §8.1 |
...
**后来 `safeBaseDir` 按需求回来了**，默认仍为空（不限制），所以上面那句告知依然成立，
只是多了 `file access scope: <路径> (safeBaseDir)` 这一种形态。恢复时的关键取舍：
...
- **挂载表与盘列表一起收窄**：列表就是枚举面，不收窄等于继续告诉他「这台机器上有什么」；
  基目录不在任何挂载点之下时补一个合成挂载点，否则「上一级」会停在范围外。
