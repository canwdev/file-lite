...
- **并发与落盘**：任务并发固定 2、单任务内文件并发固定 4，改名之前一定 `fsync` 临时文件——三者都不可配。
...
**关于 fsync**：改名之前一定 `fsync` 临时文件，使"改名即完整"在断电场景也成立；代价是大量小文件时更慢。曾经用配置项 `copyFsync` 提供关闭开关，现在固定打开：丢数据的代价比慢一点大得多。
...
| fsync 拖慢小文件 | 完整性 vs 速度 | 固定开启 fsync，宁可慢一点也不丢数据 |
