...
- **并发与落盘**：任务并发固定 2、单任务内文件并发固定 4，改名之前 `fsync` 临时文件——三者都不可配（网络位置上跳过 fsync，见 §3.5）。
...
- **测试**：
  ...
  - 前端：独立 Playwright 子项目 `e2e/`，覆盖冲突弹窗、上传冲突、进度条、运行中取消、失败清单与重试、跨窗口可见性；测试方法、用例与夹具见 `e2e/README.md`。
...
│   ├── publish.go         PublishFile：同目录临时文件 + fsync + 原子改名（网络目标跳过 fsync / 权限 / 时间）
...
复制一个文件：
  src/a.txt  →  dst/.fl-part-<12 hex>   （写入 + 计数进度）
                    │  写完
                    ├─ chmod / chtimes 对齐源文件（网络位置跳过）
                    ├─ fsync（本机卷恒开；网络位置跳过，见 3.5）
...
**关于 fsync**：改名之前 `fsync` 临时文件，使"改名即完整"在断电场景也成立；代价是大量小文件时更慢。曾经用配置项 `copyFsync` 提供关闭开关，现在**本机卷上**固定打开：丢数据的代价比慢一点大得多。

**网络位置上则跳过**（`PublishOptions.NetworkTarget`，判定见 `fileops.IsNetworkTarget`）。这里要分清跳过的到底是什么：

- **不跳的是「原子发布」本身**——临时文件 + 改名照旧，目标路径依旧不会出现半个文件。rename 在 SMB / NFS 上几乎免费。
- **跳的是三处收尾**：fsync、`chmod`、`chtimes`。它们各是一次网络往返，而换来的东西在网络位置上基本不存在：走网络协议写出去的数据对端收到即安全（SMB / NFS 关闭即提交，rclone 的 VFS 缓存等关闭后写回），对象存储则根本没有权限位。
- 代价很窄、也很明确：**网络位置上掉电后，已改名的文件内容未必已落盘**。进程被杀不受影响（内核继续写回页缓存），只有掉电 / 内核崩溃才会体现。

这条判定依赖挂载表，所以 `/proc/mounts` 的网络文件系统表必须包含「以网络为后端的 FUSE」（`fuse.rclone`、`fuse.s3fs`…）——漏判的后果不是显示难看，而是并发档位错成 64，对着对象存储同时甩出几十个请求。
...
退回刷新也只重载当前目录：保留旧列表直到新数据到达，内容一致时直接沿用旧数组（不重排、
不重渲染），只有切换目录才清空。
...
| fsync 拖慢小文件 | 完整性 vs 速度 | 本机卷固定开启，宁可慢一点也不丢数据；网络位置跳过（见 §3.5） |
